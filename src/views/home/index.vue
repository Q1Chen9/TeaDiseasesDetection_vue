<template>
	<div class="home-container layout-pd">
		<el-row :gutter="15" class="home-card-two mb15">
			...
			<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12">
				<div class="home-card-item">
					<div style="height: 100%" ref="leafGradeBarChartRef"></div>
				</div>
			</el-col>
		</el-row>
		<el-row :gutter="15" class="home-card-three" style="margin-top: 15px;">
			<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12">
				<div class="home-card-item">
					<div class="home-card-item-title">病虫害预测信息</div>
					<div class="home-monitor">
						<div class="flex-warp">
							<el-table :data="state.data.slice(0,6)" style="width: 100%">
								<el-table-column prop="username" label="用户名" align="center" />
								<el-table-column prop="label" label="病虫害名称" align="center" :formatter="formatLabel" />
			...
			<el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12">
				<div class="home-card-item">
					<div class="home-card-item-title">茶叶分级信息</div>
					<div class="home-monitor">
						<div class="flex-warp">
							<el-table :data="state.leafGradeData.slice(0,6)" style="width: 100%">
								<el-table-column prop="username" label="用户名" align="center" />
								<el-table-column prop="label" label="茶叶等级" align="center" :formatter="formatLabel" />
								<el-table-column prop="startTime" label="时间" align="center" />
							</el-table>
						</div>
					</div>
		...
		<el-row :gutter="15" class="home-card-three" style="margin-top: 15px;">
			<el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
				<div class="home-card-item">
					...
					<div class="home-monitor">
						<div class="flex-warp">
							<el-table :data="state.allPredictions.slice(0,10)" style="width: 100%">
								<el-table-column prop="username" label="用户名" align="center" />
								<el-table-column prop="type" label="预测类型" align="center" />
...
const leafGradeBarChartRef = ref();
...
const formatLabel = (row: any, column: any, cellValue: any) => {
    try {
        const labels = JSON.parse(cellValue);
        return Array.isArray(labels) ? labels.join(', ') : cellValue;
    } catch (e) {
        return cellValue;
    }
};

const combineAndSortPredictions = () => {
    const diseasePredictions = state.data.map(item => ({
        ...item,
        type: '病虫害检测',
        label: JSON.parse(item.label || '[]').join(', ') || '无'
    }));
    const gradePredictions = state.leafGradeData.map(item => ({
        ...item,
        type: '茶叶分级',
        label: JSON.parse(item.label || '[]').join(', ') || '无'
    }));
    state.allPredictions = [...diseasePredictions, ...gradePredictions]
        .sort((a, b) => new Date(b.startTime).getTime() - new Date(a.startTime).getTime());
};

const getLeafGradeData = () => {
	request({
		url: '/api/leafGradeRecords',
		method: 'get',
		params: {
			pageNum: 1,
			pageSize: 1000,
		...
		state.leafGradeData = res.data.records;
		initLeafGradeBarChart();
	})
}

const initLeafGradeBarChart = () => {
	if (!state.global.dispose.some((b: any) => b === state.global.leafGradeBarChart)) {
		if (state.global.leafGradeBarChart) state.global.leafGradeBarChart.dispose();
	}
	state.global.leafGradeBarChart = markRaw(echarts.init(leafGradeBarChartRef.value, state.charts.theme));
	const gradeCounts = {
		"1-2天": 0,
		"2-4天": 0,
		"4-7天": 0,
	...
	state.leafGradeData.forEach(record => {
		try {
			const labels = JSON.parse(record.label);
			if (Array.isArray(labels)) {
				labels.forEach(label => {
					if (gradeCounts.hasOwnProperty(label)) {
						gradeCounts[label]++;
					}
				});
			}
		} catch (e) {
			console.error("Error parsing leaf grade labels:", e);
		}
	});
	...
	const option = {
		...
		yAxis: {
			type: 'value',
			name: '数量',
			splitLine: { show: true, lineStyle: { type: 'dashed', color: '#E8F5E9' } },
			axisLabel: { color: '#1A3C34' },
		},
		series: [
			{
	...
	state.global.leafGradeBarChart.setOption(option);
	state.myCharts.push(state.global.leafGradeBarChart);
};
...
const initPieChart = () => {
	...
	const allPredictions = [...state.data, ...state.leafGradeData];
	const usernameCounts = allPredictions.reduce((acc, prediction) => {
...
const initBarChart = () => {
	...
	const categories = ['藻斑病','褐枯病','灰枯病','健康','茶盲蝽','赤叶枯病'];
...
const fetchData = () => {
	...
	Promise.all([promise1, promise2]).then(([imgRes, gradeRes]) => {
		state.data = imgRes.data.records;
		state.leafGradeData = gradeRes.data.records;

		initLineChart();
		initPieChart();
		initBarChart();
		initLeafGradeBarChart();
		combineAndSortPredictions();
	});
};
